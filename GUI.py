...
root = tk.Tk()
f = open("d:/IVth_year/2nd_Sem/Major_Project/Major_Project_dev/userFile.txt", "r")
data = f.read()
values = data.split(",")
check_val = values[3]
print(values)
print(check_val)
def popup():
    messagebox.showinfo("Warning","Please do not perform this exercise as it may lead to any injury")
    ans = messagebox.askyesno("Question","Do you still want to perform")
    return ans

def open_shoulder_press():
    print(check_val)
    if check_val == "1" or check_val == "2" or check_val == "3":
        val=popup()
    if check_val=="4":
        val=True
    if val :
...
def open_lateral_raise():
    if check_val == "2" or check_val == "3":
        val2=popup()
    if check_val == "4" or check_val=="1":
        val2 = True
    if val2:
